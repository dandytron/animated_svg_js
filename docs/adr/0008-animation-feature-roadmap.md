...
`animate.js` (SMIL preview) and `export.js` (per-frame attribute writes via
`setCurrentTime` capture). All items below were verified to survive the frame-capture
export in the prototypes.

## Proposals

### 1. Line draw-on: `trace` mode (stroke-dashoffset) — HIGH
...
### 3. Text-intro animation library — HIGH
...
### 5. Camera moves: focus zoom/pan + emphasis — MED-HIGH
...
### 7. Compositing legibility toggle — MED
